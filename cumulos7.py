...
if st.button("Procesar y Guardar"):
    ...
    if latitud and longitud:
        ...
        try:
            ...
            # Filtrar registros para el país seleccionado
            pais_lat, pais_lon = latitud, longitud  # Coordenadas obtenidas para el país seleccionado
            data = data[(data['latitud'] >= pais_lat - 5) & (data['latitud'] <= pais_lat + 5) &
                        (data['longitud'] >= pais_lon - 10) & (data['longitud'] <= pais_lon + 10)]
            ...
            # Graficar los casos diarios reales como una línea
            plt.plot(casos_diarios['dias'], casos_diarios['casos_diarios'], color='blue', marker='o', linestyle='-', label='Casos Diarios Reales')
            ...
            plt.title(f'Comparación de Casos Diarios y Ajuste por Mínimos Cuadrados (Grado 1) para {pais}')
            plt.legend()
            plt.grid()
